...
const Navbar = () => {
  const { isLoggedIn, logout, cartItems } = useContext(ShopContext);
  ...
  return (
    <div className='flex items-center justify-between py-5 font-medium'>
      ...
      {/* MIDDLE NAV */}
      <ul className='hidden sm:flex gap-5 text-sm' style={{ color: '#6f1811' }}>
        <NavLink to='/' className='flex flex-col items-center gap-1'><p>HOME</p></NavLink>
        <NavLink to='/products' className='flex flex-col items-center gap-1'><p>PRODUCTS</p></NavLink>
      ...
      <div className='flex items-center gap-6'>
        ...
        {/* Search icon */}
        <svg xmlns="http://www.w3.org/2000/svg" height="28px" viewBox="0 -960 960 960" width="28px" strokeWidth="0.5" className="cursor-pointer">
        ...
        <Link to='/cart' className='relative'>
          ...
          {/* new - real cart count from backend */}
          {cartItems.length > 0 && (
